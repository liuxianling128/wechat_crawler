...
#################################################################################################################################33
    ...
    def post(self,url,params,file = '',param_type = 'x-www-form-urlencode'):
        respone_code = None
        respone = None
        try:
            header = {'token':'','timestap':getNowTime()}
            files = {'file':open(file,'rb')}
            ret = requests.post(self.Server+url, data=params, headers=header,files = files)
    ...
    def get_selenium_js_html(self, url):
        browser = webdriver.PhantomJS(executable_path=r'E:\soft\python\tools\phantomjs-2.1.1-windows\bin\phantomjs.exe')
